<template>
  <div class='hello'>
    <div v-if='show_menu_btn' id='header-right-btn'>
      <el-dropdown trigger='click' @command='handleCommand'>
        ...
        <el-dropdown-menu slot='dropdown'>
          ...
          <el-dropdown-item v-if='projectInfo' command='new_page'>{{
              $t('new_page')
            }}
          </el-dropdown-item>
          <el-dropdown-item v-if='projectInfo' command='new_catalog'>{{
              $t('new_catalog')
            }}
          </el-dropdown-item>
          <el-dropdown-item v-if='projectInfo' command='edit_page'>{{
              $t('edit_page')
            }}
          </el-dropdown-item>
          <el-dropdown-item v-if='projectInfo' command='copy'>{{
              $t('copy')
            }}
          </el-dropdown-item>
          <el-dropdown-item v-if='projectInfo' command='ShowHistoryVersion'>
            {{ $t('history_version') }}
          </el-dropdown-item
          >
          <el-dropdown-item v-if='projectInfo' command='export'>{{
              $t('export')
            }}
          </el-dropdown-item>
          <el-dropdown-item v-if='projectInfo' command='delete_page'>{{
              $t('delete_interface')
            }}
          </el-dropdown-item>
        </el-dropdown-menu>
    ...
    <div class='op-bar' v-if='show_op_bar'>
      ...
      <span v-if='projectInfo'>
        ...
        >
          ...
          >
            <router-link
                :to="'/item/export/' + projectInfo.item_id"
                v-if='projectInfo'
            >
          ...
          >
            ...
            >
              <router-link
                  :to="'/item/setting/' + projectInfo.item_id"
                  v-if='projectInfo'
              >
    ...
    >
      ...
      <div v-show='projectInfo.item_edit'>
        <el-checkbox
...
export default {
  ...
  methods: {
    edit_page() {
      const page_id = this.page_id > 0 ? this.page_id : 0
      const url = '/page/edit/' + this.item_id + '/' + page_id
    ...
    delete_page() {
      const page_id = this.page_id > 0 ? this.page_id : 0
      const that = this
      const url = DocConfig.server + '/api/page/delete'
    ...
    CreateSiglePage() {
      const page_id = this.page_id > 0 ? this.page_id : 0
      const that = this
      const url = DocConfig.server + '/api/page/createSinglePage'
      const params = new URLSearchParams()
      params.append('page_id', page_id)
      ...
      that.axios.post(url, params).then(function(response) {
        if (response.data.error_code === 0) {
          const unique_key = response.data.data.unique_key
          if (unique_key) {
    ...
    new_page() {
      const url = '/page/edit/' + this.projectInfo.item_id + '/0'
      this.$router.push({ path: url })
    },

    mamage_catalog() {
      const url = '/catalog/' + this.projectInfo.item_id
      this.$router.push({ path: url })
    },
    showMoreAction() {
      this.showMore = true
      ...
      switch (command) {
        ...
        case 'copy':
          this.$router.push({
            path:
                '/page/edit/' +
                this.projectInfo.item_id +
                '/0?copy_page_id=' +
                this.page_id
          })
          break
        case 'export':
          this.$router.push({ path: '/item/export/' + this.projectInfo.item_id })
          break
        case 'delete_page':
          this.delete_page()
          break
  ...
  mounted() {
    ...
    if (
        this.isMobile() ||
        (window.innerWidth < 1300 && !this.projectInfo.is_login)
    ) {
